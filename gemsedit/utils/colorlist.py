...
def _build_deduplicated(source: dict[str, str], threshold: float = 35.0) -> dict[str, str]:
    """Remove near-duplicate colors from the source dict.

    Iterates in original dict order. Each color is kept only if it is
    at least ``threshold`` RGB-distance away from every previously kept color.
    """
    kept: list[tuple[str, str, tuple[int, int, int]]] = []
    for name, hexval in source.items():
        rgb = _hex_to_rgb(hexval)
        if any(_rgb_distance(rgb, prev_rgb) < threshold for _, _, prev_rgb in kept):
            continue
        kept.append((name, hexval, rgb))
...
def _build_max_spaced(source: dict[str, str]) -> dict[str, str]:
    """Reorder colors so that adjacent entries are as visually different as possible.

    Uses a greedy farthest-point approach: start with the first color,
    then always pick the remaining color that is farthest (in RGB distance)
    from the color just placed.
    """
    remaining = {name: _hex_to_rgb(hexval) for name, hexval in source.items()}
    if not remaining:
        return {}

    # Start with the first color
    first_name = next(iter(remaining))
    ordered: list[tuple[str, tuple[int, int, int]]] = [(first_name, remaining.pop(first_name))]

    while remaining:
        last_rgb = ordered[-1][1]
        # Pick the color farthest from the last one placed
        best_name = max(remaining, key=lambda n: _rgb_distance(remaining[n], last_rgb))
        ordered.append((best_name, remaining.pop(best_name)))

    return {name: source[name] for name, _ in ordered}


def _build_ultra_spaced(source: dict[str, str], lookback: int = 10) -> dict[str, str]:
    """Reorder colors for maximum visual separation from recent neighbors.

    Like ``_build_max_spaced``, but each candidate is scored by its
    *minimum* RGB distance to the last ``lookback`` placed colors.
    The candidate with the highest minimum distance wins.  This avoids
    the ping-pong effect where only the immediately previous color is
    considered.
    """
    remaining = {name: _hex_to_rgb(hx) for name, hx in source.items()}
    if not remaining:
        return {}
    ...
    while remaining:
        recent = [rgb for _, rgb in ordered[-lookback:]]
        best_name = max(
            remaining,
            key=lambda n, _recent=recent: min(_rgb_distance(remaining[n], r) for r in _recent),
        )
        ordered.append((best_name, remaining.pop(best_name)))
...
if __name__ == "__main__":
    ...
    def make_color_panel(title: str, color_dict: dict[str, str]) -> QWidget:
        ...
        header = QLabel(f"{title}  ({len(items)} colors)")
        ...
        for name, hexval in items:
            ...
            swatch = QFrame()
            swatch.setFixedSize(QSize(60, 24))
            swatch.setAutoFillBackground(True)
            pal = swatch.palette()
            pal.setColor(QPalette.ColorRole.Window, QColor(hexval))
            swatch.setPalette(pal)
            swatch.setFrameShape(QFrame.Shape.Box)

            label = QLabel(f"{name}  {hexval}")
    ...
    window = QSplitter(Qt.Orientation.Horizontal)
    window.setWindowTitle("Color List Comparison")
    ...
    for title, cdict in [
        ...
        ("Deduplicated", deduplicated_colors),
        ("Max Spaced", max_spaced_colors),
        ("Ultra Spaced", ultra_spaced_colors),
    ]:
        panel = make_color_panel(title, cdict)
        scroll = QScrollArea()
