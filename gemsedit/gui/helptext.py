...
condition_desc = {
    ...
    "IsShaded": "<b>IsShaded</b>() : This condition returns <i>true</i> if the <u>current object</u> (the one whose actions are being processed) is currently shaded. This condition can only be used on object or pocket actions.",
    "IsNotShaded": "<b>IsNotShaded</b>() : This condition returns <i>true</i> if the <u>current object</u> (the one whose actions are being processed) is not currently shaded. This condition can only be used on object or pocket actions.",
    ...
    "VarLacksString": '<b>VarLacksString</b>(Variable,SubString,Logic,CaseSensitive) : This condition returns <i>true</i> if the user-created token <b><i>Variable</i></b> does not exist or its string value does not contain <b><i>SubString</i></b>. <b><i>SubString</i></b> may be comma-separated to check for multiple substrings. <b><i>Logic</i></b> (default "or") determines whether <i>any</i> (or) or <i>all</i> (and) substrings must be missing. <b><i>CaseSensitive</i></b> (default True) controls whether the comparison is case-sensitive. Returns <i>true</i> if the variable does not exist or the substring criteria are met.',
}
...
action_desc = {
    ...
    "PortalTo": "<b>PortalTo</b>(View,VidFile,Delay) : This action causes GEMS to load <b><i>View</i></b>. If <b><i>VidFile</i></b> is provided, the video plays fullscreen as a transition effect before the view change. Right-click the video to skip. If <b><i>Delay</i></b> is greater than 0, the portal waits that many seconds before executing; during the delay, the mouse is hidden and no actions are triggered.",
    "ChangeViewImages": "<b>ChangeViewImages</b>(View,Foreground,Background) : This action changes the <b><i>Foreground</i></b> and/or <b><i>Background</i></b> images for the specified <b><i>View</i></b>. Only valid image file paths will be applied. This is intended to alter images for a view the user may travel to subsequently.",
    ...
    "ToggleObjectShade": "<b>ToggleObjectShade</b>(NameSubstring,Color) : This action toggles shading on <u>all objects</u> across <u>all views</u> whose name contains <b><i>NameSubstring</i></b>. If an object is already shaded, the shade is removed. If not shaded, a filled shade is drawn over the object's bounding shape using the specified <b><i>Color</i></b> (with alpha for transparency). Shading persists across view changes.",
    "ShadeObject": "<b>ShadeObject</b>(NameSubstring,Color) : This action draws a filled shade over <u>all objects</u> across <u>all views</u> whose name contains <b><i>NameSubstring</i></b>. The shade uses the specified <b><i>Color</i></b> (with alpha for transparency) and persists across view changes until removed with UnshadeObject.",
    "UnshadeObject": "<b>UnshadeObject</b>(NameSubstring) : This action removes any shade from <u>all objects</u> across <u>all views</u> whose name contains <b><i>NameSubstring</i></b>. If no matching objects are shaded, this action has no effect.",
}
